...
# read in/combine simulation output files
...
We performed a power analysis to characterize the power of our study design and analysis. That is, assuming there truly are differences in average minijack rates among sire ages, we wished to quantify the probability that our study would have assigned them statistical significance.
...
2. **Progeny sample size**: In 2015/2016 combined the average number of male progeny assigned to a cross was `r round(mean(n_progeny), 1)`. We used three levels of progeny sample sizes:
   * _Base_: 22
   * _Double_: 44
   * _Quadruple_: 88
3. **True effect of sire age**: We wished to represent the range of minimal sire age effects we would have deemed biologically significant if our model deemed them statistically significant, recognizing that if the model can detect small effects then, all else equal, it can detect larger effects with greater power. We used three levels of true effects represented as the expected minijack rates by sire age at the average progeny weight and in the absence of random effects:
   * _No effect_: all sire ages have expected minijack rates of 50%. This case allows us to evaluate our experiment-wise Type I error rate. 
...
*  Adding more parents to the study (conducting more crosses) gains more power than adding more progeny. For example, base parent sample size with quadruple progeny size often gave less power than double parent size with base progeny size.
*  The Type I error rate is approximately 0.25 for searching for at least 1 significant comparisons, but declines rapidly when searching for more comparisons. Further, it is largely unaffected by the amount of random variability or sample size.
